...
export function CollectionSnapshotByCreatorView (this: any, props: any) {
    ...
    //const qnconnection = new Connection(QUICKNODE_RPC_ENDPOINT);
    ...
    const fetchIndexedMintListByCreator = async(address:string) => {
        const body = {
            method: "getNFTsByCreator",//"getNFTsByCollection",
            jsonrpc: "2.0",
            params: [
              address
            ],
    ...
    const processCollection = async(updateAuthority:string) => {
        ...
        if (creatorAddress){
            ...
            setLoading(true);
            finalList = await fetchIndexedMintListByCreator(creatorAddress);
    ...
    const handleUploadToStoragePool = async () => {
        const fileName = creatorAddress.substring(0,9)+'.json';
    ...
    const processMintAddress = async(updateAuthority:string) => {
        ...
        if(mintAddress){
            ...
            const metadata = await ticonnection.getAccountInfo(pda);
            ...
            if (metadata?.data){
                try{
                    ...
                    if (meta_final.data?.creators[0]?.address){
                        setCreatorAddress(meta_final.data?.creators[0]?.address);
                    }
    ...
    return (
        ...
        >
            <Stack
                ...
                <Button
                    onClick ={() => processMintAddress(creatorAddress)}
                ...
                <TextField
                    fullWidth
                    placeholder='Enter a Creator Address'
                    value={creatorAddress}
                    onChange={(e) => setCreatorAddress(e.target.value)}/>
                <Button
                    onClick ={() => processCollection(creatorAddress)}
                    disabled={(!creatorAddress || (!fileGenerated && loading))}
                    variant='contained'
                >
                    Generate Snapshot
                </Button>
                ...
                {fileGenerated &&
                    <ButtonGroup>
                        <Tooltip title="Download Grape.art JSON file">
                            <Button
                                download={`${creatorAddress.substring(0,9)}.json`}
                                href={fileGenerated}
                            >
                                <DownloadIcon /> JSON
                            </Button>

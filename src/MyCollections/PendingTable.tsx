...
export function PendingTable({info, isAdmin, approve, wallet, refund}: {
                    ...
                    <StyledTable sx={{ minWidth: 500 }} size="small" aria-label="Approved Collections Table">
                        <TableHead sx={{ backgroundColor: 'inherit' }}>
                            <TableRow sx={{ backgroundColor: 'inherit' }}>
                                ...
                                <TableCell sx={{ minWidth: '100px', backgroundColor: 'inherit' }}>
                                    <Typography variant="caption" sx={{ marginRight: '30px' }}>Update Authority</Typography>
                                </TableCell>
                                {isAdmin && <TableCell sx={{ minWidth: '100px', backgroundColor: 'inherit' }}>
                                    <Typography variant="caption" sx={{ marginRight: '30px' }}>Approve</Typography>
                                </TableCell>}
                                {info.find((listing) => listing.listing_requester.equals(wallet)) &&  <TableCell sx={{ minWidth: '100px', backgroundColor: 'inherit' }}>
                                    <Typography variant="caption" sx={{ marginRight: '30px' }}>Refund</Typography>
                                </TableCell>}
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {info && info.map((listing:any,key:number) =>
                            <TableRow sx={{ backgroundColor: 'inherit' }} key={key}>
                                <TableCell sx={{ minWidth: '180px', backgroundColor: 'inherit' }}>
                                    <Typography variant="caption">{listing.name}</Typography>
                                </TableCell>
                                <TableCell sx={{ minWidth: '100px', backgroundColor: 'inherit' }}>
                                    <Typography variant="caption">{listing.listing_requester.toBase58()}</Typography>
                                </TableCell>
                                <TableCell sx={{ minWidth: '100px', backgroundColor: 'inherit' }}>
                                    <Typography variant="caption" sx={{ marginRight: '30px' }}>{listing.collection_update_authority.toBase58()}</Typography>
                                </TableCell>
                                {isAdmin && <TableCell sx={{ minWidth: '100px', backgroundColor: 'inherit' }}>
                                    <Button onClick={() => approve(listing.verified_collection_address || listing.collection_update_authority)}>Approve</Button>
                                </TableCell>}
                                {listing.listing_requester.equals(wallet) && <TableCell sx={{ minWidth: '100px', backgroundColor: 'inherit' }}>
                                    <Button onClick={() => refund(listing.verified_collection_address || listing.collection_update_authority)}>Request Refund</Button>
                                    </TableCell>}
                            </TableRow>
                            )}
                        </TableBody>

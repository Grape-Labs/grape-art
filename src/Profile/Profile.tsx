...
function formatBlockTime(date: string, epoch: boolean, time: boolean) {
    ...
    if (time) date_str = new Date(date).toLocaleString();
    if (epoch) {
        date_str = new Date(+date * 1000).toLocaleDateString(); //.toUTCString();
        if (time) date_str = new Date(+date * 1000).toLocaleString(); //.toUTCString();
    }
    return <>{date_str}</>;
}
...
const PubKeyDialog = (props: any) => {
    ...
    function HandlePKSubmit(event: any) {
        ...
        if (walletPKId.length >= 32 && walletPKId.length <= 44) {
    ...
    return (
        <React.Fragment>
            ...
            >
                <DialogTitle>{t('Public Key')}</DialogTitle>
                <form onSubmit={HandlePKSubmit}>
                    ...
                    <DialogActions>
                        ...
                        <Button type="submit" variant="text" title="GO">
                            {t('Go')}
                        </Button>
                    </DialogActions>
                </form>
...
const GalleryItem = (props: any) => {
    ...
    const getCollectionData = async () => {
        try {
            const meta_primer = collectionitem;
            const buf = Buffer.from(meta_primer.data, 'base64');
            const meta_final = decodeMetadata(buf);

            const metadata = await window.fetch(meta_final.data.uri).then((res: any) => res.json());
            ...
            return (
                <Grid container alignItems="center" justifyContent="center">
                    ...
                    <Grid item sx={{ display: 'flex' }}>
                        <Box sx={{ p: 1 }}>
                            <Typography variant="caption">{collectionmeta.collectionmeta?.name}</Typography>
                        </Box>
                    </Grid>
                </Grid>
            );
            ...
            <List sx={{ m: 1, p: 1 }}>
                <ListItem disablePadding>
                    <ListItemButton
                        title={t('Back Home')}
                        component={Link}
                        to={`${GRAPE_PROFILE}${publicKey.toBase58()}`}
                        sx={{
                            width: '100%',
                            borderRadius: '25px',
                            p: 1,
                            ...
                            <TabActiveProvider initialActiveKey="0">
                                ...
                                >
                                    <Tab
                                        icon={
                                            <Hidden smUp>
                                                <CollectionsOutlinedIcon sx={{ fontSize: '18px' }} />
                                            </Hidden>
                                        }
                                    ...
                                    <Tab
                                        icon={
                                            <Hidden smUp>
                                                <ArrowCircleLeftOutlinedIcon sx={{ fontSize: '18px' }} />
                                            </Hidden>
                                        }
                                    ...
                                    <Tab
                                        icon={
                                            <Hidden smUp>
                                                <GavelOutlinedIcon sx={{ fontSize: '18px' }} />
                                            </Hidden>
                                        }
                                    ...
                                    <Tab
                                        icon={
                                            <Hidden smUp>
                                                <SolCurrencyIcon sx={{ fontSize: '18px' }} />
                                            </Hidden>
                                        }
...
const GeneralProfileView = (props: any) => {
    ...
    const followWalletConnect = async (followAddress: string, solanaAddress: string) => {
        ...
        const tofollow = followAddress;
        //console.log(followAddress+": "+solanaAddress);
        const promise = await cyberConnect.connect(followAddress, solanaAddress).catch(function (error) {
            console.log(error);
        });
        getFollowStatus();
    };
    const followWalletDisconnect = async (followAddress: string) => {
        ...
        const promise = await cyberConnect.disconnect(followAddress).catch(function (error) {
            console.log(error);
        });
        getFollowStatus();
    };
    ...
    const getCollectionData = async (start: number) => {
        try {
            ...
            const mintarr = wallet_collection
    ...
    const getFollowStatus = async () => {
        if (publicKey) {
            if (pubkey) {
                ...
                const socialconnection = await fetchSearchAddrInfo(publicKey.toBase58(), pubkey);
                if (socialconnection) {
    ...
    const getCollectionMeta = async (start: number) => {
        if (!loading) {
            ...
            const wallletlength = wallet_collection.length;

            const loops = Math.ceil(wallet_collection.length / rpclimit);
            let collectionmeta: any[] = [];

            console.log('lps: ' + loops);
            for (let x = 0; x < loops; x++) {
                //const interval = setTimeout(() => {
                const tmpcollectionmeta = await getCollectionData(x);
            ...
            const finalmeta = final_collection_meta; //JSON.parse(JSON.stringify(collectionmeta));
            try {
                ...
                for (var i = 0; i < finalmeta.length; i++) {
                    ...
                    if (finalmeta[i].meta.data.symbol.length > 0) {
                        for (const metainstance of finalmeta) {
                ...
                let counter = 0;
                for (var i = 0; i < finalmeta.length; i++) {
                    if (previousSymbol !== finalmeta[i].meta.data.symbol) counter = 0;
                ...
                if (solanaDomain) {
                    ...
                    try {
                        if (profilePictureUrl)
                            sppicon =
                                '<i class="wallet-adapter-button-start-icon"><img style="border-radius:24px"" src="' +
                                profilePictureUrl +
                                '" alt="Solana Profile Icon"></i>';
                        document.getElementsByClassName('grape-wallet-button')[0].innerHTML =
                            sppicon + '<span class="wallet-adapter-solana-domain">' + solanaDomain + '</span>';
                    } catch (e) {}
                }
            }
        }
                    ...
                    >
                        {featuredObj && (
                            <Card sx={{ borderRadius: '26px', mb: 2 }}>
                                ...
                                <CardActionArea component="a" href={`${featuredObj.daourl}`} target="_blank">
                                    <CardMedia
                                        component="img"
                                    ...
                                    {featuredObj ? (
                                        ...
                                        <>
                                            {hasProfilePicture && profilePictureUrl && (
                                                <img src={profilePictureUrl} alt="" className="grape-art-profile-img" />
                                            )}
                                        </>
                                    )}
                                                    ...
                                                    {final_collection && final_collection.length > 0 && (
                                                        <>
                                                            <Typography
                                                                ...
                                                                sx={{ flexGrow: 1, mt: 3 }}
                                                            >
                                                                <strong>{final_collection.length}</strong>{' '}
                                                                {t('items collected')}
                                                            </Typography>
                                                        </>
                                                    )}
...
export function ProfileView(this: any, props: any) {
    ...
    const fetchWalletCollection = async () => {
        ...
        try {
            ...
            const wallet =
                resultValues &&
                resultValues?.map((collectionInfo: any) => {
        ...
        if (withPubKey) {
            ...
            if (ValidateAddress(withPubKey)) {
                ...
                if (withPubKey.toLocaleUpperCase().indexOf('.SOL') > -1 || withPubKey.slice(0, 1) === '@') {
                    if (withPubKey.toLocaleUpperCase().endsWith('.SOL')) {
                        // Solana Domain
                        getReverseDomainLookup(withPubKey);
                    } else if (withPubKey.toLocaleUpperCase().startsWith('@')) {
                        // Twitter Handle
                                ...
                                <Box className="">
                                    <Grid container direction="column" spacing={2} alignItems="center" rowSpacing={8}>
                                        <Grid item xs={12} alignItems="center">
                                            <Typography variant="h3" color="inherit" display="flex" sx={{ mt: 2 }}>
                                                <img
                                                    src={MARKET_LOGO}
                                                    width="300px"
                                        ...
                                        <Grid item xs={12} alignItems="center">
                                            <Typography variant="h6" color="inherit" display="flex">
                                                {t('Social. Stateless. Marketplace.')}
                                            </Typography>
                                        </Grid>
                                    </Grid>
